...
void SystemClass::Run()
{
	...
	while(!done)
	{
		...
		else
		{
			...
			if (m_Input->IsNumPad0Down())
				m_Graphics->SetData(0);
			if (m_Input->IsNumPad1Down())
				m_Graphics->SetData(1);
			if (m_Input->IsNumPad2Down())
				m_Graphics->SetData(2);
			if (m_Input->IsNumPad3Down())
				m_Graphics->SetData(3);
